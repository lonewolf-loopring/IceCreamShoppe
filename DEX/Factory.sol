 ...
 */
contract ArtDexFactory is Ownable {
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, "ArtDex: FORBIDDEN");
        feeTo = _feeTo;
    }
    ...
    /**
     * @dev Creates a liquidity pair for two tokens.
     * In the future, this is where you will link $ART2, $BRUSH, etc.
     */
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, "ArtDex: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "ArtDex: ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "ArtDex: PAIR_EXISTS");
